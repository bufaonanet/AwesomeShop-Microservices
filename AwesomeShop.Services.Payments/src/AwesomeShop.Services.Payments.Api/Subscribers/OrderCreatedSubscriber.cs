...
public class OrderCreatedSubscriber : BackgroundService
{
    ...
    private const string RoutingKey = "log.*";
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.Received += async (sender, eventArgs) =>
        {
            ...
            var paymentProcessed = await ProcessPayment(message);
    ...
    private async Task<bool> ProcessPayment(OrderCreated orderCreated)
    {
        ...
        await invoiceRepository.AddAsync(
            new Invoice(orderCreated.TotalPrice, orderCreated.Id, orderCreated.PaymentInfo.CardNumber));
